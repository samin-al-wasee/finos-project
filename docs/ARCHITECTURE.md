...
- **transactions** — Income/expense/transfer entry, listing, balance impact, and search/filter (FR-02). `TransactionFilter` is a pure predicate over already-loaded rows — text search plus account, category, type, and date-range criteria — kept deliberately single-dimension; compound criteria like an amount range combined with a category and month are Phase 2's "advanced" search (docs/ROADMAP.md §8.5).
