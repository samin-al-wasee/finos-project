...
- **transactions** — Income/expense/transfer entry, listing, balance impact, and search/filter, including the docs/ROADMAP.md §8.5 "advanced" search (FR-02, built ahead of its Phase 2 slot with explicit authorization). `TransactionFilter` is a pure predicate over already-loaded rows — text search, account, category, type, date range, and an amount range all combine with AND — so "food transactions between ৳500 and ৳2,000 in July" is several ordinary criteria at once, not a distinct compound-query feature. A saved-query/report builder (docs/ROADMAP.md §8.5, docs/DATA_MODEL.md §57) is also built: a new `SavedQueries` table (schema v9) stores a name plus `TransactionFilter.toJson()`'s structured criteria — the free-text search box is deliberately excluded, since it's typed continuously rather than configured. Applying a saved query loads its criteria into the filter sheet's own fields for the user to review (and optionally adjust) before pressing Apply, the same "review before committing" shape as using a template (§8.2) — it never bypasses the sheet to apply itself directly.
...
- **reports** — Income vs expense and expense-by-category for a selected calendar period, compared against the immediately preceding period (docs/ROADMAP.md §8.4, built ahead of its Phase 2 slot with explicit authorization). The period is either one of the fixed choices (this/last month, this/last year) or an arbitrary custom range, via the `ReportWindowSelection` sealed type (`FixedReportPeriod` / `CustomReportRange`) — each variant derives its own comparison window and labels, so `reportDataProvider` and the screen work in terms of the selection rather than branching on which kind it is. A custom range's comparison window is the immediately preceding range of equal length. Reuses the transaction DAO's existing period-aggregation queries rather than adding new ones. Also includes a Budget Performance section listing every active budget's standing, most-at-risk first (`budgetsForPerformanceReport`); each budget keeps its own natural current window instead of the report's selected period, since a budget's period doesn't generally align with it — this reuses `budgetProgressProvider` unchanged, so budget calculation logic itself is untouched. A Cash Flow by Account section (`accountCashFlowsForReport`) lists each active account with activity that period alongside its net for the immediately preceding period; deliberately no percent-change figure here, since net cash flow is signed and a percent change is undefined once the sign flips between periods — the existing per-account net display follows the same amount-plus-color convention the report's own Net card already uses. A Monthly Spending section (`monthlySpendingProvider`/`monthlySpendingWindows`) shows a fixed trailing six-month expense trend, independent of the period selector, since a multi-month trend doesn't fit the single-window comparison shape the rest of the screen shares.
- **templates** — Saved presets for manual transaction entry (FR-02-adjacent, docs/ROADMAP.md §8.2, built ahead of its Phase 2 slot with explicit authorization; see docs/DATA_MODEL.md §56). A new `TransactionTemplates` table (schema v7) where every field but `name` and `type` is nullable — a template is a partial preset, not a complete transaction. Using one pre-fills `TransactionFormScreen` via its new `template` parameter (mutually exclusive with `initial`, the existing edit path); the user still reviews and saves like any other new transaction, always dated today regardless of when the template was made. Deliberately not recurring transactions (§8.1): a template never creates anything by itself.
- **recurring** — Rules for subscriptions and regular bills that generate transactions on confirmation (docs/ROADMAP.md §8.1, built ahead of its Phase 2 slot with explicit authorization; see docs/DATA_MODEL.md §§26–28). A new `RecurringTransactions` table (schema v8) where, unlike a template, `amountMinor` and `accountId` are required — a rule computes what is due unattended, so it cannot leave those blank. Due occurrences are never created silently: `dueOccurrences()` computes what is owed as of now (capped at `maxDueOccurrences` per rule but always reporting the true `totalCount`, so a backlog is disclosed rather than truncated invisibly) and the user explicitly confirms or skips before anything is written, one rule's backlog at a time via `confirmNext`/`confirmAll`/`skipAll`, run inside a single `AppDatabase.transaction` so a rule can never end up with only part of its backlog processed. An occurrence's amount/account/category are not editable at confirm time (V1 simplification) — to change one, skip it and enter a transaction manually. Deliberately excludes an arbitrary custom interval (only daily/weekly/monthly/yearly) and any transaction-to-rule backlink column, following the templates precedent rather than loans': deleting a rule never touches transactions it already generated.
- **quick_entry** — A terminal-style single-line bar, pinned to the bottom of the Transactions tab, for recording any write operation the app supports (docs/ROADMAP.md §8.8, built directly at user request rather than from anything sketched elsewhere in the roadmap). `QuickCommandSpec` (docs `quick_command_spec.dart`) declares each of the eleven operations' fields in the order typed tokens fill them; `parseQuickEntry` maps a raw line onto the currently-chosen spec purely by token position, and never treats a still-being-typed `@partial` token as a real value. Owns no persistence of its own — resolving a parsed command (`resolveQuickCommand`) only ever produces a lightweight, unsaved `*Draft` value (one per target entity: transaction, loan, account, category, budget, template, recurring rule) or, for a loan repayment, a loan id plus a draft for the now-public `RepaymentDialog` (previously private to `LoanDetailsScreen`). Every dispatch opens that entity's *existing* create screen pre-filled via a new `draft` constructor parameter — added alongside each screen's existing `initial` (edit) parameter, the same "review before saving" shape templates and saved filters already use — so no entity's validation or save logic is duplicated. A resolved command validates only enough to land on the right screen (a name must resolve to a real account/category/loan, an amount must parse); anything the target screen's own Save button already checks, such as a non-zero amount, is deliberately left to it.
